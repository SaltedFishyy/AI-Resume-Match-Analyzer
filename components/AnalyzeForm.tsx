...
export function AnalyzeForm() {
  ...
  const canAnalyze = resumeText.trim().length >= 100 && jobDescription.trim().length >= 100;
  ...
  return (
    <>
      <form className="mt-10" onSubmit={handleSubmit}>
        ...
        <div className="surface-card mt-5 grid gap-5 rounded-2xl p-5 sm:grid-cols-2 sm:items-center lg:grid-cols-[1.4fr_repeat(4,1fr)]">
          ...
          {[[Target,"Match score","bg-blue-50 text-blue-700","text-blue-950"],[Clock3,"Missing keywords","bg-rose-50 text-rose-700","text-rose-950"],[Sparkles,"Skill gaps","bg-orange-50 text-orange-700","text-orange-950"],[Lightbulb,"Suggestions","bg-emerald-50 text-emerald-700","text-emerald-950"]].map(([Icon,label,tone,labelTone]) => { const FeatureIcon = Icon as typeof Target; return <div key={label as string} className="flex items-center gap-3"><span className={`flex h-10 w-10 items-center justify-center rounded-full ${tone as string}`}><FeatureIcon className="h-5 w-5" /></span><span className={`text-xs font-semibold ${labelTone as string}`}>{label as string}</span></div>; })}
        </div>
        ...
        <div className="surface-card mt-5 flex flex-col gap-3 rounded-2xl p-4 sm:flex-row sm:items-center">
          <div className="flex flex-1 items-center gap-3 px-2 text-xs text-slate-500"><ShieldCheck className="h-5 w-5 text-slate-400" /><span>Your analysis is saved to your account history.</span></div>
          <button type="submit" disabled={isLoading || !canAnalyze} className="focus-ring order-first inline-flex min-w-56 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-7 py-3.5 font-semibold text-white shadow-lg shadow-blue-500/25 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0 sm:order-none">{isLoading ? "Analyzing..." : "Analyze resume"}{!isLoading && <ArrowRight className="h-4 w-4" />}</button>
...
function InputCard({ icon: Icon, tone, title, description, count, maximum, sampleLabel, onSample, footerMiddle, children }: InputCardProps) {
  const remainingCharacters = Math.max(100 - count, 0);
  const toneClass = tone === "blue" ? "bg-blue-50 text-blue-700" : "bg-violet-50 text-violet-700";

  return <section className="surface-card rounded-3xl p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div className="flex gap-4"><span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${toneClass}`}><Icon className="h-5 w-5" /></span><div><label htmlFor={tone === "blue" ? "resumeText" : "jobDescription"} className="text-lg font-bold text-slate-950">{title}</label><p className="mt-1 text-sm leading-6 text-slate-500">{description}</p></div></div><span className="hidden items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600 sm:flex"><ShieldCheck className="h-3.5 w-3.5" /> Account saved</span></div><div className="mt-6">{children}</div><div className="mt-3 grid gap-3 rounded-xl border border-slate-100 bg-slate-50/70 px-3 py-2.5 sm:grid-cols-3 sm:items-center"><span className="text-xs text-slate-500 sm:text-left">{remainingCharacters > 0 ? `${remainingCharacters.toLocaleString()} more characters needed · ${count.toLocaleString()} / ${maximum.toLocaleString()} characters` : `${count.toLocaleString()} / ${maximum.toLocaleString()} characters`}</span><div className="flex min-w-0 justify-center">{footerMiddle}</div><div className="flex justify-center sm:justify-end"><button type="button" onClick={onSample} className="focus-ring inline-flex items-center gap-2 rounded-lg px-3 py-1.5 text-xs font-bold text-blue-600 transition hover:bg-blue-50"><FileText className="h-3.5 w-3.5" /> {sampleLabel}</button></div></div></section>;
}
